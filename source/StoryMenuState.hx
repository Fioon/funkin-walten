...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var redGradient:FlxSprite = new FlxSprite().loadGraphic(Paths.image('storybg'));
		add(redGradient);
		var storyctrl:FlxSprite = new FlxSprite().loadGraphic(Paths.image('storyctrl'));
		add(storyctrl);
		
		scoreText = new FlxText(10, 10, 0, "SCORE: 49324858", 36);
		scoreText.setFormat(Paths.font("bauhaus-heavy-bt.ttf"), 32);

		txtWeekTitle = new FlxText(FlxG.width * 0.7, 10, 0, "", 32);
		txtWeekTitle.setFormat(Paths.font("bauhaus-heavy-bt.ttf"), 32, FlxColor.WHITE, RIGHT);
		...
		bgPoster = new FlxSprite(-100, 0);
		bgPoster.antialiasing = ClientPrefs.globalAntialiasing;
		...
		var tracksSprite:FlxSprite = new FlxSprite(FlxG.width * 0.04, 56 + 300).loadGraphic(Paths.image('Menu_Tracks'));
	...
	function changeDifficulty(change:Int = 0):Void
	{
		...
		if(sprDifficulty.graphic != newImage)
		{
			...
			sprDifficulty.x += (-3700 - sprDifficulty.width) / 3;
			sprDifficulty.alpha = 0;
			sprDifficulty.y = leftArrow.y - 80;

			if(tweenDifficulty != null) tweenDifficulty.cancel();
			tweenDifficulty = FlxTween.tween(sprDifficulty, {y: leftArrow.y - 80, alpha: 1}, 0.07, {onComplete: function(twn:FlxTween)
			{
	...
	function changeWeek(change:Int = 0):Void
	{
		...
		bgPoster.visible = true;
		var assetName:String = leWeek.weekBackground;
		if(assetName == null || assetName.length < 1) {
			bgPoster.visible = false;
		} else {
			bgPoster.loadGraphic(Paths.image('storymenuposters/menu_' + assetName));
		}
	...
	function updateText()
	{
		...
		txtTracklist.x -= FlxG.width * 0.38;
